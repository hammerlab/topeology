...
int max_alignment_score(const char *seq_a, const char *seq_b) {
...
IntVal alignment_score_impala(FunctionContext* context,
                              const StringVal& string_a,
                              const StringVal& string_b) {
  std::string string_a_str((const char *)string_a.ptr,string_a.len);
  std::string string_b_str((const char *)string_b.ptr,string_b.len);

  const char *seq_a = string_a_str.c_str();
  const char *seq_b = string_b_str.c_str();
  
  int max_score = max_alignment_score(seq_a, seq_b);

  return IntVal(max_score);
}
